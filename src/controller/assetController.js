...
const newAssets = async (req, res) => {
  try {
    ...
    if (!id_asset || !name) {
      return res
        .status(400)
        .json({ message: "id_asset and name are required." });
    }
...
const updateAssets = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Asset updated successfully.", data: targetAsset });
  } catch (err) {
...
const deleteAssets = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Asset soft-deleted successfully." });
  } catch (err) {
...
const syncAssets = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: `Sinkronisasi ${totalSynced} aset berhasil` });
  } catch (err) {
    console.error("Sync error:", err.message);
    return res
      .status(500)
      .json({ message: "Gagal sinkronisasi aset", error: err.message });
  }
};
